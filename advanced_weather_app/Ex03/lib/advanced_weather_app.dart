                              ...
                              Expanded(
                                child: ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    final time =
                                        _hourlyWeatherData!['time'][index];
                                    final temperature =
                                        _hourlyWeatherData!['temperature_2m']
                                            [index];
                                    final weatherCode =
                                        _hourlyWeatherData!['weathercode']
                                            [index];
                                    final windSpeed =
                                        _hourlyWeatherData!['windspeed_10m']
                                            [index];
...
class HourlyTemperatureChart extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    List<charts.Series<TimeTemp, DateTime>> series = [
      charts.Series(
        id: "Temperature",
        data: List.generate(
          24,
          (index) => TimeTemp(
            DateTime.parse(times[index]),
            double.parse(temperatures[index].toString()),
          ),
        ),
        domainFn: (TimeTemp data, _) => data.time,
        measureFn: (TimeTemp data, _) => data.temp,
    ...
    return charts.TimeSeriesChart(
      ...
      primaryMeasureAxis: const charts.NumericAxisSpec(
        tickProviderSpec: charts.StaticNumericTickProviderSpec(
          [
            charts.TickSpec(0, label: '0°C'),
            charts.TickSpec(10, label: '10°C'),
            charts.TickSpec(20, label: '20°C'),
            charts.TickSpec(30, label: '30°C'),
          ],
        ),
      ),
